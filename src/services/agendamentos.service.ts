...
export class AgendamentosService {
  ...
  async criar(dados: {
    ...
    dataHoraInicio: string;
    dataHoraFim: string;
    // duracaoMinutos: REMOVIDO - coluna gerada
    valor: number;
    valorDesconto?: number;
    // valorFinal: REMOVIDO - coluna gerada
    observacoes?: string;
    ...
    try {
      ...
      // IMPORTANTE: 
      // - duracao_minutos: calculado automaticamente (data_hora_fim - data_hora_inicio)
      // - valor_final: calculado automaticamente (valor - valor_desconto)
      // NÃO enviar esses campos no insert!
      const { data, error } = await supabase
        .from('agendamentos')
        .insert({
  ...
  async atualizar(
    ...
    try {
      ...
      if (updates.profissionalId && updates.dataHoraInicio && updates.dataHoraFim) {
        const disponibilidadeCheck = await this.verificarDisponibilidade(
          updates.profissionalId,
          updates.dataHoraInicio,
          updates.dataHoraFim,
          agendamentoId
        );
      ...
      if (updates.profissionalId) updateData.profissional_id = updates.profissionalId;
      if (updates.produtoId) updateData.produto_id = updates.produtoId;
      if (updates.dataHoraInicio) updateData.data_hora_inicio = updates.dataHoraInicio;
      if (updates.dataHoraFim) updateData.data_hora_fim = updates.dataHoraFim;
      if (updates.valor !== undefined) updateData.valor = updates.valor;
      if (updates.valorDesconto !== undefined) updateData.valor_desconto = updates.valorDesconto;
      if (updates.observacoes !== undefined) updateData.observacoes = updates.observacoes;
      if (updates.observacoesInternas !== undefined) updateData.observacoes_internas = updates.observacoesInternas;
      if (updates.status) updateData.status = updates.status;
      if (updates.confirmado !== undefined) updateData.confirmado = updates.confirmado;

      // Nunca atualizar colunas geradas
      delete updateData.duracao_minutos;
      delete updateData.valor_final;
